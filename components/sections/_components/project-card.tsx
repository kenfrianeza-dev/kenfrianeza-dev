      ...
      <div className="flex flex-col h-full overflow-hidden rounded-2xl border border-surface-800/60 bg-surface-900/40 backdrop-blur-md transition-all duration-500 hover:border-accent-500/30 hover:bg-surface-800/60">
        ...
        <div className="relative aspect-video w-full overflow-hidden bg-surface-950/50">
          ...
          <div className="absolute inset-0 bg-surface-950/60 opacity-0 backdrop-blur-sm transition-all duration-500 group-hover:opacity-100 hidden md:flex items-center justify-center gap-4">
            {project.liveUrlDisabled ? (
              <div className="relative group/tooltip">
                <button
                  disabled
                  aria-label="Live Preview — not available"
                  className="flex h-12 w-12 translate-y-8 cursor-not-allowed items-center justify-center rounded-full bg-surface-700/60 text-surface-500 opacity-0 transition-all duration-500 delay-100 group-hover:translate-y-0 group-hover:opacity-100"
                >
                  <ExternalLink className="h-5 w-5" />
                </button>
                <span className="pointer-events-none absolute -top-10 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-surface-800 border border-surface-700 px-2.5 py-1 text-[11px] text-surface-300 opacity-0 transition-opacity duration-200 group-hover/tooltip:opacity-100">
        ...
        <div className="flex flex-1 flex-col p-6">
          <div className="mb-2 flex items-start justify-between gap-4">
            ...
            {project.status && (
              <span
                className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider border ${project.status === "done"
              ...
              >
                <span className="relative flex h-1.5 w-1.5">
                  ...
                  <span
                    className={`relative inline-flex h-1.5 w-1.5 rounded-full ${project.status === "done" ? "bg-emerald-400" : "bg-amber-500"
                      }`}
                  ></span>
                </span>
                {project.status}
          ...
          <div className="mt-6 flex items-center gap-3 md:hidden">
            {project.liveUrlDisabled ? (
              <div className="relative flex flex-1 group/tooltip">
                <button
                  disabled
                  className="flex flex-1 cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-surface-800/50 border border-surface-700/40 py-2.5 text-sm font-semibold text-surface-500"
                >
                  <ExternalLink className="h-4 w-4" />
                  Preview
                </button>
                <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-surface-800 border border-surface-700 px-2.5 py-1 text-[11px] text-surface-300 opacity-0 transition-opacity duration-200 group-hover/tooltip:opacity-100">
                  Live preview not available yet
                </span>
              </div>
            ) : (
